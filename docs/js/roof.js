    ...
    function calculateVolume () {
      ...
      this.problemList = [
        {"calc": function(valuesList){
          sig = '';
          return sigFig(roundTo((4/3)*Math.PI*valuesList[0]**3, 2), getSigFig(valuesList[0]));
          },
          "text": "Calculate, approximately, the area of a sphere with radius $r=" +String(valuesList[0])+ "\\ \\mathrm{cm}$ . The area of a sphere is determined by " + "$\\frac{4}{3}\\pi r^{3}$"
        },
      ]
      ...
      this.possibleAnswersHTML = function() {
        ...
        if (answerList[0].length != 2) {
          for (i = 0; i < answerList.length; i++) {
            tempHTML += ("<li class='answer' answer='"+answerList[i]+"'>" + String(answerList[i]) + "$\\ \\mathrm{cm}$" + "</li>");
          };
          tempHTML += "</ul>";
          return tempHTML;
        };
